...
class Crawl():
    ...
    def __setter(self, ips):
        try:
            for item in ips:
                self.redis.hash_set_original(item, list(item.keys())[0])
        except:
            pass
    ...
    def __template_for_daili_func(self):
        def parser(response):
            ...
            for ip in ips:
                proxy.append({'http': f'http://{str(ip)}'})
                proxy.append({'https': f'https://{str(ip)}'})
            return proxy
        website = 'website'
    ...
    def __kuaidaili(self):
        def parser(response):
            ...
            for i in zip(ips, ports):
                proxy.append({'http': f'http://{str(i[0])}:{str(i[1])}'})
                proxy.append({'https': f'https://{str(i[0])}:{str(i[1])}'})
            return proxy
    ...
    def __xiladaili(self):
        def parser(response):
            ...
            for ip_port in ip_ports:
                proxy.append({'http': f'http://{str(ip_port)}'})
                proxy.append({'https': f'https://{str(ip_port)}'})
            return proxy
